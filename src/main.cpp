#include <Ticker.h>
#include <Wire.h>
#include <LittleFS.h>
#include <base64.h>

#include <SPI.h>

#include <Adafruit_GFX.h>    // Core graphics library
#include <Adafruit_ST7735.h> // Hardware-specific library for ST7735
#include <Adafruit_NeoPixel.h> // LED Ring library

#include <ESP8266WiFi.h>
#include <ESP8266HTTPClient.h>
#include <ESP8266WebServer.h>
#include <ESP8266httpUpdate.h>
#include <AutoConnect.h>
#include <CertStoreBearSSL.h>
#include <time.h>

// LED Ring pin and count
#define LED_PIN D1
#define LED_COUNT 12

// Screen dimensions and pin assignments
#define HEIGHT 128
#define WIDTH 160
#define LID D0
#define BACKLIGHT D2
#define TFT_CS         D8
#define TFT_RST        D4
#define TFT_DC         D3  // Also A0 on the TFT board
Adafruit_ST7735 tft = Adafruit_ST7735(TFT_CS, TFT_DC, TFT_RST);

#undef AUTOCONNECT_URI
#define AUTOCONNECT_URI         "/_ac/config"

// Neopixel ring instance
Adafruit_NeoPixel ring(LED_COUNT, LED_PIN, NEO_GRB + NEO_KHZ800);

// WiFi specific variables
ESP8266WebServer Server;
BearSSL::CertStore certStore;
HTTPClient http;
AutoConnect Portal(Server);
AutoConnectConfig PortalConfig;
boolean serverAvailable = true;
boolean wifiConnected = false;

// If running in debug mode, opt for using http over https
#ifdef DEBUG
  #pragma message ( "Building in Debug mode. HTTPS disabled." )
  WiFiClient *client;
  String protocol = "http://";
#else
  BearSSL::WiFiClientSecure *client;
  String protocol = "https://";
#endif

// Set the address to make http requests to
#ifdef HOST_ADDRESS
  #pragma message(HOST_ADDRESS)
  String host = HOST_ADDRESS;
#else
  #pragma message("Using default host address")
  String host = "messagebox.unitfi.com";
#endif

// Set the current version of firmware, and default to 0.0.0
#ifdef FIRMWARE_VERSION
  String version = FIRMWARE_VERSION;
#else
  #error "Firmware version not defined! Set the FIRMWARE_VERSION environment variable and try again."
#endif

const char * headerKeys[] = {"user", "code", "Content-Type", "status"};
const size_t numberOfHeaders = 4;

// Name of file to save image to
String imageName = "message.tft";

// Unique 32-byte key generated by box for authentication
// TODO: Deprecate the key string in favor of the binary version
String key;
uint8_t key_bin[32];

// User ID from server for retrieving message. Presence of user indicates
// we've registered with the server
String user;

// One-time activation code used to register the box
String code;

// Indicates if we've already completed some action for the lid opening, preventing
// multiple screen writes, clearing messages right away, etc
boolean state = false;

// Interrupt and corresponding variables for rate-limiting
Ticker timer;
volatile int httpCheck = 0;
volatile unsigned long updateCheck = 0;
volatile int lidCheck = 0;
volatile int ledCheck = 0;

// Function declarations
void onTime();
void setClock();
void downloadMessage();
void displayWiFiLogin();
void displayConnecting();
void displayWiFiConnected();
void displayActivationCode();
void displayNoServer();
void displayMessage();
void drawHeart(int, int);


void resetBox() {
  LittleFS.remove("key.txt");
  LittleFS.remove("user.txt");
  ESP.restart();
}

void setup(void)
{
  #ifdef DEBUG
    Serial.begin(9600);
    Serial.println("Starting messagebox");
    Serial.println(FIRMWARE_VERSION);
  #endif

  // Start LED Ring, assigning random colors to each LED during setup
  ring.begin();
  uint32_t color = ring.Color(15, 15, 255);
  for(int i=0; i<ring.numPixels(); i++){
    color = ring.Color(random(255), random(255), random(255));
    ring.setPixelColor(i,color);
  }
  ring.setBrightness(1);
  ring.show();

  //Intitialize the lcd screen, and show "splash screen"
  pinMode(BACKLIGHT, OUTPUT);
  pinMode(LID, INPUT);
  tft.initR(INITR_BLACKTAB);
  tft.setRotation(1);
  tft.fillScreen(ST77XX_BLACK);
  tft.setTextColor(0xFFFF);
  tft.setCursor(50, 60);
  tft.print("MessageBox");
  digitalWrite(BACKLIGHT, HIGH);
  delay(3000);

  // Initialize timer for lcd and http
  timer1_attachInterrupt(onTime);
  timer1_enable(TIM_DIV256, TIM_EDGE, TIM_LOOP); // 80MHz Base / 256 = 312.5KHz -> 3.2us
  timer1_write(3125);  // 3.2us * 3125 ~= 10000us -> 10ms

  // Inform the user we're attempting to connect to WiFi
  displayConnecting();

  // Start the wifi autoconnect service
  PortalConfig.apid = "Messagebox";
  PortalConfig.bootUri = AC_ONBOOTURI_HOME;
  PortalConfig.homeUri = "/_ac/config";
  PortalConfig.psk = "";
  PortalConfig.portalTimeout = 1;
  PortalConfig.retainPortal = true;
  Portal.config(PortalConfig);
  Portal.begin();

  // Wait for the box to be connected to WiFi, informing user of how to log in
  displayWiFiLogin();
  while(WiFi.status() != WL_CONNECTED){
    Portal.handleClient();
    delay(100);
  }

  // Bring down the Access Point
  #ifdef DEBUG
    Serial.println("Successfully connected to WiFi");}
  #endif
  WiFi.softAPdisconnect(true);
  displayWiFiConnected();

  // Prepare clock and SSL Certificates
  #ifdef DEBUG
    Serial.println("Preparing SSL");}
  #endif
  setClock();

  // Clear the screen and draw a heart
  tft.fillScreen(ST77XX_BLACK);
  drawHeart((160 / 2) - 8, (128/2) - 8);

  // Start filesystem for Authentication credentials and SSL Certs
  LittleFS.begin();

  int numCerts = certStore.initCertStore(LittleFS, PSTR("certs.idx"), PSTR("certs.ar"));

  if(numCerts == 0) {
    #ifdef DEBUG
      Serial.println("Missing certs!");}
    #endif
    // TODO: Inform the user with what has actually gone wrong. This usually
    // happens when certs have not been loaded into the filesystem
    return;
  }

  #ifdef DEBUG
    client = new WiFiClient();
  #else
    client = new BearSSL::WiFiClientSecure();
    client->setCertStore(&certStore);
    client->setInsecure();
  #endif

  // Check if a generated key exists. If it does not exist,
  // Create a new one (randomly generated 32-byte key)
  if (LittleFS.exists("key.txt"))
  {
    File keyFile = LittleFS.open("key.txt", "rb");
    for( int i = 0; i < 32; i++)
    {
      key_bin[i] = keyFile.read();
      key += String(key_bin[i], HEX);
    }
    keyFile.close();
  } else {
    key = "";
    for( int i = 0; i < 32; i++)
    {
      uint8_t c = random(255);
      key += String(c, HEX);
      key_bin[i] = c;
    }
    File keyFile = LittleFS.open("key.txt", "wb");
    keyFile.write(key_bin, 32);
    keyFile.close();
  }

  // Check if we have a User ID saved. Presence of User ID
  // indicates we've successfully been registered.
  if (LittleFS.exists("user.txt"))
  {
    File userFile = LittleFS.open("user.txt", "r");
    user = userFile.readString();
  }
  else
  {
    user = "";
  }
}

void loop(void)
{
  // Run screen and LED Ring functions
  if(lidCheck >= 5){
    lidCheck = 0;
    if(digitalRead(LID) == LOW){
      ring.setBrightness(0);
      ring.show();
      // If we haven't already completed some action for this instance of the lid being removed,
      // Try to display a message first. If there is no message, show any other issues, otherwise
      // Show a heart to indicate the box is active but there is nothing to do.
      if(!state){
        if(LittleFS.exists(imageName)){
          #ifdef DEBUG
            Serial.println("Displaying Message.");
          #endif
          displayMessage();
          LittleFS.remove(imageName);
          ring.setBrightness(0);
          ring.show();
        } else if(user == "" && code != "") {
          displayActivationCode();
          #ifdef DEBUG
            Serial.println("Unit not activated. Displaying activation code.");
          #endif
        } else if(!serverAvailable){
          displayNoServer();
          #ifdef DEBUG
            Serial.println("Server not available.");
          #endif
        } else {
          tft.fillScreen(ST77XX_BLACK);
          drawHeart((160 / 2) - 8, (128/2) - 8);
          #ifdef DEBUG
            Serial.println("Nothing to display.");
          #endif
        }
      }
      state = true;
      digitalWrite(BACKLIGHT, HIGH);
    } else {
      // Lid is on, turn the backlight off
      digitalWrite(BACKLIGHT, LOW);
      state = false;
    }
  }

  if(httpCheck >= 1000){
    httpCheck = 0;
    if(LittleFS.exists(imageName)){
      //We have a message waiting to be read. Do nothing.
      //httpCheck = 0;
    } else if(user != "") {
      // Begin asking the server if we have any new messages
      http.begin(*client, String(protocol + host + "/message/read") +
                      "?key=" + key +
                      "&user=" + user);
      http.collectHeaders(headerKeys, numberOfHeaders);
      int httpCode = http.GET();

      if(httpCode >= 200 && httpCode <= 299){
        serverAvailable = true;
        #ifdef DEBUG
          Serial.println("Successfully contacted server");
        #endif
      } else {
        serverAvailable = false;
        #ifdef DEBUG
          Serial.println("Server contact failure: " + String(httpCode));
        #endif
      }

      // Check status and react accordingly
      if(http.hasHeader("status")){
        String status = http.header("status");
        if(status == "No Messages"){
          //No message, do nothing.
          #ifdef DEBUG
            Serial.println("No Messages.");
          #endif
        } else if(status == "New Message" && http.header("Content-Type") == "application/octet-stream"){
          downloadMessage();
          uint32_t color = ring.Color(15, 15, 255);
          for(int i=0; i<ring.numPixels(); i++){
            ring.setPixelColor(i,color);
          }
          ring.setBrightness(64);
          ring.show();
        } else if(status == "No Box"){
          #ifdef DEBUG
            Serial.println("Resetting box");
          #endif
          resetBox();
        }
      }
      http.end();
    } else {
      // We haven't authenticated yet, send credentials to server until
      // we get a user id
      http.begin(*client, String(protocol + host + "/box/activate") +
                      "?key=" + key +
                      "&firmware=" + version);

      http.collectHeaders(headerKeys, numberOfHeaders);
      int httpCode = http.GET();

      if(httpCode >= 200 && httpCode <= 299){
        serverAvailable = true;
      } else {
        serverAvailable = false;
      }

      // Check activation headers:
      // If "code", display the activation code
      // If "user", we've been acivated and assigned to a user,
      //    so save that information.
      if(http.hasHeader("code") && code == ""){
        code = http.header("code");
        state = false;
      } else if(http.hasHeader("user")){
        user = http.header("user");
        state = false;
      } else if(http.hasHeader("reset")){
        resetBox();
      }

      http.end();
    }
  }

  if(updateCheck >= 100000){
    updateCheck = 0;
    // Check if we have a new update
    String test = base64::encode(key_bin, 32, false);
    ESPhttpUpdate.setAuthorization(test);
    t_httpUpdate_return ret = ESPhttpUpdate.update(*client, String(protocol + host + "/box/update"), version);
    switch(ret){
      case HTTP_UPDATE_FAILED:
        #ifdef DEBUG
          Serial.printf("HTTP_UPDATE_FAILED Error (%d): %s\n", ESPhttpUpdate.getLastError(), ESPhttpUpdate.getLastErrorString().c_str());
          Serial.println(HOST_ADDRESS);
        #endif
        tft.fillScreen(ST77XX_BLACK);
        tft.setCursor(30, 50);
        tft.println("Update failed");
        break;

      case HTTP_UPDATE_NO_UPDATES:
        break;

      case HTTP_UPDATE_OK:
        #ifdef DEBUG
          Serial.println("HTTP_UPDATE_OK");
        #endif
        tft.fillScreen(ST77XX_BLACK);
        tft.setCursor(30, 50);
        tft.println("Update successful");
        break;
    }
  }

  delay(5);
}

void ICACHE_RAM_ATTR onTime() {
  httpCheck++;
  lidCheck++;
  updateCheck++;
}

void downloadMessage(){
  #ifdef DEBUG
    Serial.println("Downloading message.");
  #endif

  if(LittleFS.exists(imageName)){
    LittleFS.remove(imageName);
  }

  File image = LittleFS.open(imageName, "w");
  if(!image){
    //Image open failed!
    return;
  }

  int len = http.getSize();
  #ifdef DEBUG
    Serial.print("Download size: "); Serial.println(http.getSize());
  #endif
  WiFiClient* stream = http.getStreamPtr();
  while(http.connected() && (len > 0 || len == -1)){
    size_t size = stream->available();
    for(size_t i = 0; i < size; i++){
      image.write(stream->read());
      int c = 1;
      if(len > 0){
        len -= c;
      }
    }
    delay(1);
  }
  image.close();
  #ifdef DEBUG
    Serial.println("Download complete.");
  #endif
  uint32_t color = ring.Color(15, 15, 255);
  for(int i=0; i<ring.numPixels(); i++){
    ring.setPixelColor(i,color);
  }
  ring.setBrightness(64);
  ring.show();
}

void displayWiFiLogin(){
  tft.fillScreen(ST77XX_BLACK);
  tft.setCursor(0, 50);
  tft.println("   Connect your phone to");
  tft.println("   the 'Messagebox' WiFi");
  tft.println("   network to set me up!");
}

void displayConnecting(){
  tft.fillScreen(ST77XX_BLACK);
  tft.setCursor(30, 50);
  tft.println("Connecting to WiFi");
}


void displayWiFiConnected(){
  tft.fillScreen(ST77XX_BLACK);
  tft.setCursor(10,60);
  tft.println("WiFi Network Connected!");
}

void displayActivationCode(){
  tft.fillScreen(ST77XX_BLACK);
  tft.setCursor(0,40);
  tft.println("       Activate me @");
  tft.println(host + "/register");
  tft.println("    in your browser.");
  tft.println("");
  tft.println("      Activation Code:");
  tft.println("         " + code);
}

void displayNoServer(){
  tft.fillScreen(ST77XX_BLACK);
  tft.setCursor(10,70);
  tft.println("Can't contact MessageBox");
  tft.setCursor(70,80);
  tft.println(":(");
  tft.println(host);
}

void displayMessage(){
  File messageFile = LittleFS.open(imageName, "r");
  #ifdef DEBUG
    Serial.print("File size: "); Serial.println(messageFile.size());
  #endif
  uint16_t pixel;
  for(int i = 0; i < HEIGHT; i++){
    for(int j = 0; j < WIDTH; j++){
      pixel = messageFile.read();
      pixel += messageFile.read() << 8;
      tft.drawPixel(j, i, pixel);
    }
  }
}

void drawHeart(int x, int y){
  tft.fillCircle(x+4, y+4, 4, 0xB882);
  tft.fillCircle(x+12, y+4, 4, 0xB882);
  tft.fillTriangle(x, y+4, x+16, y+4, x+8, y+16, 0xB882);
}

// Set time via NTP, as required for x.509 validation
void setClock() {
  configTime(3 * 3600, 0, "pool.ntp.org", "time.nist.gov");
  #ifdef DEBUG
    Serial.print("Waiting for NTP time sync: ");
  #endif
  time_t now = time(nullptr);
  while (now < 8 * 3600 * 2) {
    delay(500);
    #ifdef DEBUG
      Serial.print(".");
    #endif
    now = time(nullptr);
  }
  #ifdef DEBUG
    Serial.println("");
  #endif
  struct tm timeinfo;
  gmtime_r(&now, &timeinfo);
  #ifdef DEBUG
    Serial.print("Current time: ");
    Serial.print(asctime(&timeinfo));
  #endif
}
